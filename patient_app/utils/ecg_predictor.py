...
class ECGPredictor:
    ...
    def calculate_confidence(self, probas_malade):
        ...
        return float(np.mean(certainty_scores))  # Convertir en float Python
...
Une consultation médicale rapide est nécessaire pour évaluer ces anomalies."""
    ...
    def analyze_personal_ecg(self, cycles):
        ...
        try:
            ...
            # 8. Préparation des résultats détaillés - Convertir tous les types numpy
            cycles_details = []
            for i, (proba, classification) in enumerate(zip(probas_malade, classifications)):
                cycles_details.append({
                    'cycle_num': int(i + 1),  # Convertir en int Python
                    'probability': float(proba),  # Convertir en float Python
            ...
            results = {
                ...
                'confidence_score': float(self.calculate_confidence(probas_malade)),  # S'assurer que c'est un float Python
            ...
            if risk_assessment['risk_level'] in ['MEDIUM', 'HIGH']:
                try:
                    ...
                    results['pathology_confidence'] = float(pathology_results['diagnostic_principal']['pourcentage'] / 100)  # Convertir en float Python
    ...
    def _analyze_model_2_pathology(self, cycles, target_length=182):
        ...
        try:
            ...
            # Diagnostic final - Convertir tous les types numpy
            ...
            return {
                'total_cycles': int(len(cycles)),  # Convertir en int Python
                'pathologies': {
                    "type": [int(t) for t in pathologies_results["type"]],  # Convertir chaque élément
                    "probabilites": [float(p) for p in pathologies_results["probabilites"]],  # Convertir en float Python
                    "probas_par_classe": [[float(p) for p in pred] for pred in pathologies_results["probas_par_classe"]]  # Convertir tous les éléments
                },
                'diagnostic_principal': {
                    'type': int(main_type),  # Convertir en int Python
                    'nombre_cycles': int(main_type_count),  # Convertir en int Python
                    'pourcentage': float(main_type_percentage)  # Convertir en float Python
                }
            }
    ...
    def _generate_pathology_interpretation(self, pathology_results):
        """
        Génère une interprétation textuelle simplifiée des résultats de pathologie
        """
        type_mapping = {
            0: "Normal",
            1: "Battement Ectopique Supraventriculaire",
        ...
        interpretation = f"""🔍 Type de pathologie principal : {()}
...
- Des examens complémentaires seront nécessaires pour confirmer le diagnostic précis"""
        ...
        if main_type == 1:  # Battement Ectopique Supraventriculaire
            interpretation += "\n- Surveillance des battements cardiaques supraventriculaires"
        elif main_type == 2:  # Battement Ectopique Ventriculaire
            interpretation += "\n- Évaluation approfondie des battements ventriculaires ectopiques"
        elif main_type == 3:  # Battement de Fusion
            interpretation += "\n- Analyse détaillée des battements de fusion"
        elif main_type == 4:  # Inconnu
            interpretation += "\n- Des examens complémentaires sont fortement recommandés"
    ...
    def _calculate_detailed_stats(self, probas_malade, classifications):
        ...
        return {
            ...
            'total_cycles': int(len(classifications)),  # Convertir en int Python
            ...
            'consecutive_abnormal': int(self._count_consecutive_abnormal(classifications))  # Convertir en int Python
        }
